...
public class BookJPARepository implements IBookRepository {
    ...
    private EntityManager em = EntityManagerFactorySingleton.getEMF().createEntityManager();
    ...
    public List<Book> getByTitle(String title) throws SQLException {
        return em.createQuery("select b from Book b where upper(b.title) like '%"
                + title.toUpperCase() + "%'").getResultList();
    }
